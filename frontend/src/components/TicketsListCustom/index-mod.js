...
const TicketsListCustom = (props) => {
  ...
  useEffect(() => {
    dispatch({ type: "RESET" });
    setPageNumber(1);
  }, [status, searchParam, dispatch, showAll, tags, users, selectedQueueIds, isGroup]);
  ...
  useEffect(() => {
    ...
    // if (profile === "user" && Departamento == null) {
    // ocultar a fila cinza para usuario comum
    if (profile === "user") {
      dispatch({ type: "LOAD_TICKETS", payload: filteredTickets });
    } else {
  ...
  }, [status, showAll, user, selectedQueueIds, tags, users, profile, queues, isGroup]);
          ...
          {ticketsList.length === 0 && !loading ? (
            ...
            <>
              {ticketsList.map((ticket) => (
                <TicketListItem ticket={ticket} key={ticket.id} showWhatsappConnection={true} />
              ))}
            </>
          )}
